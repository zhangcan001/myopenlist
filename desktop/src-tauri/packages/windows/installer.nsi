...
!macro CheckAllOpenListProcesses
  ...
  !else
    nsis_tauri_utils::FindProcess "openlist-desktop.exe"
  !endif
  Pop $R0
  ${If} $R0 = 0
    DetailPrint "Kill openlist-desktop.exe..."
    !if "${INSTALLMODE}" == "currentUser"
      nsis_tauri_utils::KillProcessCurrentUser "openlist-desktop.exe"
    !else
      nsis_tauri_utils::KillProcess "openlist-desktop.exe"
    !endif
  ${EndIf}

  ; Check if openlist.exe is running
  !if "${INSTALLMODE}" == "currentUser"
    nsis_tauri_utils::FindProcessCurrentUser "openlist.exe"
  !else
    nsis_tauri_utils::FindProcess "openlist.exe"
  !endif
  Pop $R0
  ${If} $R0 = 0
    DetailPrint "Kill openlist.exe..."
    !if "${INSTALLMODE}" == "currentUser"
      nsis_tauri_utils::KillProcessCurrentUser "openlist.exe"
    !else
...
Section Install
  ...
  SetRegView 64  
  ReadRegStr $R2 HKCU "$R1" "OpenList Desktop"
  ${If} $R2 != ""
    DeleteRegValue HKCU "$R1" "OpenList Desktop"
  ${EndIf}

  ReadRegStr $R2 HKLM "$R1" "OpenList Desktop"
  ${If} $R2 != ""
    DeleteRegValue HKLM "$R1" "OpenList Desktop"
  ${EndIf}

  ; Delete old files before installation
    ; Delete openlist.desktop
  IfFileExists "$INSTDIR\OpenList Desktop.exe" 0 +2
    Delete "$INSTDIR\OpenList Desktop.exe"
...
Section Uninstall
  ...
  ; Delete openlist-desktop.desktop
  IfFileExists "$INSTDIR\OpenList Desktop.exe" 0 +2
    Delete "$INSTDIR\OpenList Desktop.exe"
