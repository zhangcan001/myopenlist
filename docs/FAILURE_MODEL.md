...
| F015 concurrent 401 storm | multiple requests enter refresh path | `RECOVERING` | RefreshCoordinator coalesces refresh, rejects stale responses, and shares the result | one in-flight refresh; bounded network/server retry | Reconnecting 115 | No | H |
| F016 RefreshToken refresh succeeds | accepted response passes generation + refresh-token CAS | return to prior check | atomically replace Token Pair; retry original request once; notify callback once | no duplicate refresh | Connected | No | M |
| F017 RefreshToken invalid | 401/40140116 or equivalent auth-required response | `AUTH_REQUIRED` | fast-fail the current Token Pair and preserve diagnostic status | no automatic infinite retry; reset only on a new Token Pair | 115 authorization required | Yes, scan/authorize again | H |
| F018 115 API rate limit | HTTP/API 429 with optional `Retry-After` | `DEGRADED`/`OPEN` | stop refresh attempts until server-provided or bounded fallback cooldown | no immediate retry | 115 is rate-limiting requests | No | H |
| F019 temporary 115 server error | HTTP/API 5xx or typed temporary error | `DEGRADED`/`OPEN` | bounded exponential backoff with jitter, then one half-open recovery probe | max 3 default attempts; 15 s circuit cooldown | 115 temporarily unavailable | No | M |
